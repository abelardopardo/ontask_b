...
def update_filter(request, pk):
    """
    Update the content of the filter.
    :param request: HTTP request
    :param pk: action id
    :return: HTTP response (refresh the edit action page)
    """

    # Try to get the workflow first
    workflow = get_workflow(request)
    if not workflow:
        # Workflow not found. Not good
        return redirect('workflow:index')
    ...
    # If the form is not valid or formula is empty, notify and reload
    if not form.is_valid() or not form.cleaned_data['filter_formula']:
        messages.error(request, 'Filter cannot have an empty expression')
        return redirect(reverse('action:edit', kwargs={'pk': action.id}))

    # The condition is new if no value is given
    is_new = cond_filter is None

    if is_new:
        condition_id = -1
    else:
        condition_id = cond_filter.id
    ...
    if is_new:
        ...
        condition = form.save(commit=False)
        condition.action = action
        condition.is_filter = True
    else:
        condition = form.save()

    condition.formula = json.loads(form.cleaned_data['filter_formula'])
    condition.save()
