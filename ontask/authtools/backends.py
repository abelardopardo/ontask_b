...
class CaseInsensitiveUsernameFieldBackendMixin(object):
    """
    This authentication backend assumes that usernames are email addresses
    and changes a username to lower case before an attempt is made to
    authenticate said username using a superclass authenticate method. This
    superclass should be either a user-defined authentication backend,
    or a Django-provided authentication backend (e.g., ModelBackend).

    Example usage:
        See CaseInsensitiveUsernameFieldBackend, below.

    NOTE: A word of caution. Use of this backend presupposes a way to ensure
    ...
    def authenticate(self, request, username=None, password=None, **kwargs):
        ...
        return super(
            CaseInsensitiveUsernameFieldBackendMixin, self).authenticate(
                request,
                username=username,
                password=password,
                **kwargs)
